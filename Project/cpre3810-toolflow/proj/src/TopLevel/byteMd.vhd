...
begin
    ...
    o_ByteOut <=
        ...
        std_logic_vector(resize(signed(i_mem(7 downto 0)), DATA_WIDTH))
            when i_ByteOp = "1000" else
        -- lbu: zero extend bottom byte of mem
        std_logic_vector(resize(unsigned(i_mem(7 downto 0)), DATA_WIDTH))
            when i_ByteOp = "1001" else
        -- lh: sign extend bottom halfword of mem
        std_logic_vector(resize(signed(i_mem(15 downto 0)), DATA_WIDTH))
            when i_ByteOp = "1010" else
        -- lhu: zero extend bottom halfword of mem
        std_logic_vector(resize(unsigned(i_mem(15 downto 0)), DATA_WIDTH))
