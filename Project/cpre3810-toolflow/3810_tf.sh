...
#@brief This script provides a basic runner for the 3810 Toolflow
#
...
if [ -z ${python3+x} ]; then
    if [ -f "$PYTHON3_VDI" ]; then
        ...
        echo -e "${RED}Python Missing... Try running this with PYTHON=path/to/python3 ./3810_tf.sh${NC}"
        exit 1
    fi
else
    if [ -f "$PYTHON" ]; then
...
if [ "$1" == "new" ]; then
	...
	export SALT_LICENSE_SERVER=1717@io.ece.iastate.edu
        ...
        cp -r "internal/boilerplate_riscv/"* "proj/riscv"
    ...
    if [ ! -n "$(ls -A proj/src/*.vhd 2>/dev/null )" ]; then
        echo -e "${RED}Missing proj/src directory (or no vhd files present)${NC}"
        exit 1
    fi
    if [ ! -n "$(ls -A proj/riscv/*.s 2>/dev/null)" ]; then
        echo -e "${RED}Missing proj/riscv directory (or no s files present)${NC}"
        exit 1
    fi
    if [ ! -n "$(ls -A proj/test/*.vhd 2>/dev/null)" ]; then
        echo -e "${RED}Missing proj/test directory (or no vhd files present)${NC}"
        exit 1
    fi
    if [ ! -f "proj/"*".pdf" ]; then
    ...
    echo "=================================================================================================="
    echo "   _____ _____  _____    ______   ____   ___  __   ___    _______          _  __ _                "
    echo "  / ____|  __ \|  __ \  |  ____| |___ \ / _ \/_ | / _ \  |__   __|        | |/ _| |               "
    echo " | |    | |__) | |__) | | |__      __) | (_) || || | | |    | | ___   ___ | | |_| | _____      __ "
    echo " | |    |  ___/|  _  /  |  __|    |__ < > _ < | || | | |    | |/ _ \ / _ \| |  _| |/ _ \ \ /\ / / "
    echo " | |____| |    | | \ \  | |____   ___) | (_) || || |_| |    | | (_) | (_) | | | | | (_) \ V  V /  "
    echo "  \_____|_|    |_|  \_\ |______| |____/ \___/ |_| \___/     |_|\___/ \___/|_|_| |_|\___/ \_/\_/   "
    echo "=================================================================================================="
    echo "Welcome to CPR E 3810 Toolflow."
    ...
    echo " "
    echo -e " \t 2) Build your RISCV processor in the created 'proj' directory using your editor of choice. "
    echo "You may create subdirectoryies in both the RISCV and the SRC folder to encompass your tests."
    echo " "
    echo -e " \t 3) Test your RISCV processor frequently as you go. The toolflow supports testing for both"
    ...
    echo "your RISCV processor using the Altera Quartus toolflow. Be aware... This synthesis often takes"
    ...
    echo " "
    echo "./3810_tf.sh synth"
    echo " "
    echo -e "${ORANGE}To generate/check submission: ${NC}"
    echo " "
    echo "./3810_tf.sh submit"
    echo "./3810_tf.sh submit hw"
    echo "./3810_tf.sh submit sw"
fi
